...
@property (weak, nonatomic) id<UUChartDataSource> dataSource;
...
-(void)setUpChart{
	if (self.chartStyle == UUChartStyleLine) {
        ...
        //显示竖直线
        if ([self.dataSource respondsToSelector:@selector(chart:showVerticalLineAtIndex:)]) {
            [_lineChart setShowverticalLine:[self.dataSource chart:self showVerticalLineAtIndex:0]];
        }
        //是否显示y轴Label
        if ([self.dataSource respondsToSelector:@selector(isShowYLabel)]) {
            [_lineChart setShowYlabel:[self.dataSource isShowYLabel]];
        }
        //是否显示X轴Label
        if ([self.dataSource respondsToSelector:@selector(isShowXLabel)]) {
            [_lineChart setShowXlabel:[self.dataSource isShowXLabel]];
        }
        //显示横线
