...
@property (nonatomic ,strong) NSMutableArray *layerArray;//折线数组
@property (nonatomic ,strong) NSMutableArray *linePointArray;//折线路径数组
@property (nonatomic ,strong) NSMutableArray *xLayerArray;//横线数组
@property (nonatomic ,strong) NSMutableArray *yLayerArray;//竖线数组
...
- (NSMutableArray *)xLayerArray{
    if (!_xLayerArray) {
        self.xLayerArray = [NSMutableArray array];
    }
    return _xLayerArray;
}

- (NSMutableArray *)yLayerArray{
    if (!_yLayerArray) {
        self.yLayerArray = [NSMutableArray array];
    }
    return _yLayerArray;
}
...
- (void)setYLabels:(NSArray *)yLabels
{
    ...
    for (CAShapeLayer *layer in self.xLayerArray) {
    ...
    for (int i=0; i<6; i++) {
        if ([_showHorizonLine[i] integerValue]>0) {
            ...
            [self.xLayerArray addObject:shapeLayer];
...
//    }
    ...
    for (CAShapeLayer *layer in self.yLayerArray) {
        [layer removeFromSuperlayer];
    }
    ...
    if (_showverticalLine) {
        for (int i=0; i<xLabels.count+1; i++) {
            CAShapeLayer *shapeLayer = [CAShapeLayer layer];
            UIBezierPath *path = [UIBezierPath bezierPath];
            [self.yLayerArray addObject:shapeLayer];
...
- (void)setShowYlabel:(BOOL)showYlabel{
    _showYlabel = showYlabel;
}

- (void)setShowXlabel:(BOOL)showXlabel{
    _showXlabel = showXlabel;
}
...
{
    //移除路径上面的点
    for (UIBezierPath *Path in self.linePointArray) {
    ...
    for (int i=0; i<_yValues.count; i++) {
        ...
        [self.layerArray addObject:_chartLine];
        UIBezierPath *progressline = [UIBezierPath bezierPath];
        [self.linePointArray addObject:progressline];
